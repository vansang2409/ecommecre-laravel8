<main id="main">
		<div class="container">
			...
			<!--On Sale-->
			@if($productsSale->count() > 0 && $sale->status == 1 && $sale->sale_date > Carbon\Carbon::now())
                   ...
                   @foreach($productsSale as $productSale)
					<div class="product product-style-2 equal-elem ">
						<div class="product-thumnail">
							<a href="{{route('product.details',['slug'=>$productSale->slug])}}" title="{{$productSale->name}}">
								<figure><img src="{{asset('assets/images/products')}}/{{$productSale->image}}" width="800" height="800" alt="{{$productSale->name}}"></figure>
							</a>
							<div class="group-flash">
								<span class="flash-item sale-label">sale</span>
							</div>
							<div class="wrap-btn">
								<a href="{{route('product.details',['slug'=>$productSale->slug])}}" class="function-link">quick view</a>
							</div>
						</div>
						<div class="product-info">
							<a href="{{route('product.details',['slug'=>$productSale->slug])}}" class="product-name"><span>{{$productSale->name}}</span></a>
							<div class="wrap-price"><ins><p class="product-price">${{$productSale->sale_price}}</p></ins> <del><p class="product-price">${{$productSale->regular_price}}</p></del></div>
						</div>
					</div>
                   @endforeach
				   {{-- 
						...
						<div class="tab-contents">
							<div class="tab-content-item active" id="digital_1a">
								<div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
									...
									<div class="product product-style-2 equal-elem ">
										<div class="product-thumnail">
											<a href="{{route('product.details',['slug'=>$productLatest->slug])}}" title="{{$productLatest->name}}">
												<figure><img src="{{asset('assets/images/products')}}/{{$productLatest->image}}" width="800" height="800" alt="{{$productLatest->name}}"></figure>
											</a>
											<div class="group-flash">
												<span class="flash-item new-label">new</span>
											</div>
											<div class="wrap-btn">
												<a href="{{route('product.details',['slug'=>$productLatest->slug])}}" class="function-link">quick view</a>
											</div>
										</div>
										<div class="product-info">
											<a href="{{route('product.details',['slug'=>$productLatest->slug])}}" class="product-name"><span>{{$productLatest->name}}</span></a>
											<div class="wrap-price"><span class="product-price">${{$productLatest->regular_price}}</span></div>
										</div>
									</div>
									@endforeach
                                    {{-- 
				...
				<div class="wrap-products">
					<div class="wrap-product-tab tab-style-1">
						...
						<div class="tab-contents">
                            @foreach($categories as $key => $category)
							   @php
                                    $products=DB::table('products')->where('category_id',$category->id)->get()->take($numberofproducts);
							   @endphp
